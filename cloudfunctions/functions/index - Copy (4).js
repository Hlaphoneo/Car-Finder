...
exports.addHopeRef = functions.https.onRequest((request, response) => {
  ...
  let userreference = data.userreference;
  let uid = {
    [data.userreference] : 1
  } ;
  let reportId = data.key
        admin.database().ref('/UNVERIFIED/'+reportId+"/hopes").once('value',(data)=>{  //checking if  the user already liked
              if(!data.hasChild(userreference)){ //if there use has not liked the post yet
                  admin.database().ref('/UNVERIFIED/'+reportId+"/hopes").update(uid).then(snapshot => {
                            admin.database().ref('/UNVERIFIED/'+reportId+"/hope")
                            .transaction(count => {
                                  return (count || 0) + 1;
                            })
                    })
              }else{
                  // console.log('already liked');  //return the status code here , to let the app continue
              }
        },(error)=>{
          // console.log(error);
        });
